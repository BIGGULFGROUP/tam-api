        ...
        foreach ($nichesToProcess as $targetNiche) {
            ...
            $stories = Video::where('status', 'published')
                ...
                ->map(fn ($v) => [
                    ...
                    'url' => \App\Support\PublicUrl::to("/{$v->niche}/{$v->slug}"),
            ...
            if ($dryRun) {
                $this->info("[DRY RUN] Niche '{$targetNiche}': would send to " . count($recipientList) . " subscribers with " . count($stories) . " stories. Sample link: " . $stories[0]['url']);
                continue;
            }
